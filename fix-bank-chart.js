...
(function patchBankChart() {
  ...
  function applyPatch() {
    ...
    window._renderBankChart = function bankChartFixed() {
      ...
      // ── CRITICAL FIX: Y-axis scaling yang lebih baik ──
      // SEBELUM: minRange = midPrice * 0.008 — terlalu kecil untuk saham murah
      // SESUDAH: minimum 1.5% ATAU minimum absolute 10 point, mana yang lebih besar
      const rawHi = Math.max(...candles.map(c => c.h));
      const rawLo = Math.min(...candles.map(c => c.l));
      const midPrice = (rawHi + rawLo) / 2;
      const minRangePct = midPrice * 0.015;  // 1.5% minimum
      const minRangeAbs = Math.max(10, midPrice * 0.005); // absolute floor
      const minRange = Math.max(minRangePct, minRangeAbs);
